...
function updateTagStatusBarItem(status: vscode.StatusBarItem, tagsList: PartialMatch[], position: number)
{
...
export function activate(context: vscode.ExtensionContext) {
  	...
  	context.subscriptions.push(vscode.window.onDidChangeTextEditorSelection((evt) =>
	{
  	...
  	context.subscriptions.push(vscode.commands.registerCommand('highlight-matching-tag.jumpToMatchingTag', jumpToMatchingTag));
  	context.subscriptions.push(vscode.commands.registerCommand('highlight-matching-tag.selectPairContents', selectPairContents));
	...
	const elementCompletionProvider = vscode.languages.registerCompletionItemProvider('dfdl', {
		...
		// a simple completion item which inserts `Hello World!`
			...
			*/
			...
			const xsComplexTypeName = new vscode.CompletionItem('xs:complexType name=');
			xsComplexTypeName.insertText = new vscode.SnippetString('<xs:complexType Name="$1">\n\t$0\n</xs:complexType>');
			xsComplexTypeName.documentation = new vscode.MarkdownString("Starts a complex type definition");

			const xsSimpleType = new vscode.CompletionItem('xs:simpleType');
			xsSimpleType.insertText = new vscode.SnippetString('<xs:simpleType>\n\t$0\n</xs:simpleType>');
			xsSimpleType.documentation = new vscode.MarkdownString("Starts a complex type definition");

			const xsSimpleTypeName = new vscode.CompletionItem('xs:simpleType name=');
			xsSimpleTypeName.insertText = new vscode.SnippetString('<xs:simpleType Name="$1"$0');
			xsSimpleTypeName.documentation = new vscode.MarkdownString("Starts a complex type definition");
			...
			xsChoice.insertText = new vscode.SnippetString('<xs:choice$1>\n\t$0\n</xs:choice>');
	...
	const attributeCompletionProvider = vscode.languages.registerCompletionItemProvider(
		{ language: 'dfdl'},
		{
			provideCompletionItems(document: vscode.TextDocument, position: vscode.Position) {
				...
				if(!checkBraceOpen(document, position) && !wholeLine.includes('assert') && !nearestOpenItem.includes('none')) {
					if(nearestOpenItem.includes('element')) {
						...
						xmlType.insertText = new vscode.SnippetString(preVal + 'type=\"${1|xs:string,xs:decimal,xs:float,xs:double,xs:integer,xs:nonNegativeInteger,xs:int,xs:unsignedInt,xs:short,xs:unsignedShort,xs:long,xs:unsignedLong,xs:byte,xs:unsignedByte,xs:hexBinary,xs:boolean' + additionalItems + '|}\"$0');
						...
						dfdlByteOrder.insertText = new vscode.SnippetString(preVal + "byteOrder=\"${1|bigEndian,littleEndian|}\"$0");
						...
						const dfdlChoiceBranchKey = new vscode.CompletionItem('dfdl:choiceBranchKey=');
						dfdlChoiceBranchKey.insertText = new vscode.SnippetString('dfdl:choiceBranchKey="$1"$0');
						dfdlChoiceBranchKey.documentation = new vscode.MarkdownString('List of DFDL String Literals');
					...
					if(nearestOpenItem.includes('sequence')) {
						...
						const dfdlHiddenGroupRef = new vscode.CompletionItem('dfdl:hiddenGroupRef=');
						dfdlHiddenGroupRef.insertText = new vscode.SnippetString(preVal + "dfdl:hiddenGroupRef=\"$1\"$0");
						dfdlHiddenGroupRef.documentation = new vscode.MarkdownString("Specifies ");

						const dfdlSequenceKind = new vscode.CompletionItem('dfdl:sequenceKind=');
						dfdlSequenceKind.insertText = new vscode.SnippetString(preVal + 'dfdl:SequenceKind ="${1|ordered,unordered|}"$0');
						dfdlSequenceKind.documentation = new vscode.MarkdownString('');

						const dfdlSeparator = new vscode.CompletionItem('dfdl:separator=');
						dfdlSeparator.insertText = new vscode.SnippetString(preVal + 'dfdl:separator="$1"$0');
						dfdlSeparator.documentation = new vscode.MarkdownString('');

						const dfdlSeparatorPosition = new vscode.CompletionItem('dfdl:separatorPosition=');
						dfdlSeparatorPosition.insertText = new vscode.SnippetString(preVal + 'dfdl:separatorPosition="${1|infix,postfix.prefix|}$0');
						dfdlSeparatorPosition.documentation = new vscode.MarkdownString('');

						const dfdlSeparatorSuprressionPolicy = new vscode.CompletionItem('dfdl:separatorSuppressionPolicy');
						dfdlSeparatorSuprressionPolicy.insertText = new vscode.SnippetString(preVal + 'dfdl:separatorSuppressionPolicy="${1|anyEmpty,never,trailingEmptytrailingEmptyStrict|}"$0');
						dfdlSeparatorSuprressionPolicy.documentation = new vscode.MarkdownString('');
			
						if (checkLastItemOpen(document, position) && (wholeLine.includes('<xs:sequence') || checkSequenceOpen(document, position))) {
							return [
								dfdlHiddenGroupRef,
								dfdlSequenceKind,
								dfdlSeparator,
					...
					if(wholeLine.includes('choice')) {
						var preVal = '';
						const dfdlChoiceLengthKind = new vscode.CompletionItem('dfdl:choiceLengthKind=');
						dfdlChoiceLengthKind.insertText = new vscode.SnippetString(preVal + 'dfdl:choiceLengthKind="${1|explicit,Implicit|}"$0');
						dfdlChoiceLengthKind.documentation = new vscode.MarkdownString("Valid values are implicit and explicit");

						const dfdlChoiceLength = new vscode.CompletionItem('dfdl:choiceLength=');
						dfdlChoiceLength.insertText = new vscode.SnippetString(preVal + 'dfdl:choiceLength="$1"$0');
						dfdlChoiceLength.documentation = new vscode.MarkdownString("Only used when dfdl:choiceLengthKind is explicit");

						const dfdlInitiatedContent = new vscode.CompletionItem('dfdl:intiatedContent=');
						dfdlInitiatedContent.insertText = new vscode.SnippetString(preVal + 'dfdl:intiatedContent="${1|yes,no\}"$0');
						dfdlInitiatedContent.documentation = new vscode.MarkdownString('yes indicates all branches of a choice are initiated\no indicates the branch dfdl:initator property may be ste to empty string');

						const dfdlChoiceDispatchKey = new vscode.CompletionItem('dfdl:choiceDispatchKey=');
						dfdlChoiceDispatchKey.insertText = new vscode.SnippetString('dfdl:choiceBranchKey="$1"$0');
						dfdlChoiceDispatchKey.documentation = new vscode.MarkdownString('The expression must evaluate to a string');
					...
					if(wholeLine.includes('simpleType') || checkSimpleTypeOpen(document, position)) {
						var preVal = "";
						const dfdlLength = new vscode.CompletionItem('dfdl:length=');
						dfdlLength.insertText = new vscode.SnippetString(preVal + "dfdl:length=\"$1\"$0");
						dfdlLength.documentation = new vscode.MarkdownString("length can be an expression that resolves to an unsigned integer, or a literal unsigned integer");
						...
						const dfdlSimpleTypeA = new vscode.CompletionItem('dfdl:simpleType');
						dfdlSimpleTypeA.insertText = new vscode.SnippetString('<xs:annotation>\n\t<xs:appinfo source="http://www.ogf.org/dfdl/">\n\t\talignment="$1"\n\t</xs:appinfo>\n</xs:annotation>$0');

						const dfdlSimpleTypeR = new vscode.CompletionItem('dfdl:simpleType');
						dfdlSimpleTypeR.insertText = new vscode.SnippetString('<xs:annotation>\n\t<xs:appinfo source="http://www.ogf.org/dfdl/">\n\t\trepresentation="${1|binary,|"\n\t</xs:appinfo>\n</xs:annotation>$0');

						const xsRestriction = new vscode.CompletionItem('xs:restriction');
						xsRestriction.insertText = new vscode.SnippetString('<xs:restriction base="${1|xs:string,xs:decimal,xs:float,xs:double,xs:integer,xs:nonNegativeInteger,xs:int,xs:unsignedInt,xs:short,xs:unsignedShort,xs:long,xs:unsignedLong,xs:byte,xs:unsignedByte,xs:hexBinary,xs:boolean|}"/>$0');
...
//Checks if the line at the current position in the last opened tag
function checkLastItemOpen(document: vscode.TextDocument, position: vscode.Position){
  ...
  const previousLine = document.lineAt(lineNum).text.substr(0,document.lineAt(lineNum - 1).range.end.character);
...
function lineCount(document: vscode.TextDocument, position: vscode.Position) {
	...
	function nearestOpen(document: vscode.TextDocument, position: vscode.Position) {
		var lineNum = position.line;
		while(lineNum !== -1) {
		  const wholeLine = document.lineAt(lineNum).text.substring(0,document.lineAt(lineNum).range.end.character);
			if(wholeLine.includes('element') && !wholeLine.includes('/>')) {
				...
				if(wholeLine.includes('<xs:group') && (!wholeLine.includes("</xs:group")) && (!wholeLine.includes('/>')) && (!wholeLine.includes('/'))) {
					return 'group';
				}
			}
	...
	function checkSequenceOpen(document: vscode.TextDocument, position: vscode.Position) {
		var lineNum = position.line;
		while(lineNum !== 0) {
			const wholeLine = document.lineAt(lineNum).text.substr(0,document.lineAt(lineNum).range.end.character);
			if((wholeLine.includes('<xs:sequence') && ((wholeLine.includes("</xs:sequence")) || (wholeLine.includes('/>')))) || (wholeLine.includes('</xs:sequence>'))) {
				return false;
			}
			if(wholeLine.includes('<xs:sequence') && (!wholeLine.includes("</xs:sequence")) && (!wholeLine.includes('/>'))) {
				return true;
			}
			-- lineNum;
		}
	...
	function checkSimpleTypeOpen(document: vscode.TextDocument, position: vscode.Position) {
		var lineNum = position.line;
		while(lineNum !== 0) {
			const wholeLine = document.lineAt(lineNum).text.substr(0,document.lineAt(lineNum).range.end.character);
			...
			// 	return false;
			// }
			if(wholeLine.includes('<xs:simpleType') && (!wholeLine.includes("</xs:simpleType")) && (!wholeLine.includes('/>'))) {
				return true;
			}
			-- lineNum;
		}
	...
	function checkBraceOpen(document: vscode.TextDocument, position: vscode.Position) {
		var lineNum = position.line;
	...
	const closeElementGtProvider = vscode.languages.registerCompletionItemProvider(
		'dfdl',
		{
			provideCompletionItems(document: vscode.TextDocument, position: vscode.Position) {
				...
				if (wholeLine.endsWith('>') && (wholeLine.includes('xs:element') || nearestOpenItem.includes('element') 
				  ...
				  if(wholeLine.endsWith('>') && (wholeLine.includes('xs:element ref') || wholeLine.includes('xs:group ref'))) {
						vscode.window.activeTextEditor?.insertSnippet(new vscode.SnippetString(' />\n$0'), backpos);
					}
					else if(wholeLine.endsWith('>') && (wholeLine.includes('xs:element') || (nearestOpenItem.includes('element')))) {
						vscode.window.activeTextEditor?.insertSnippet(new vscode.SnippetString('>\n\t$0\n</xs:element>'), backpos);
					}
					else if(wholeLine.endsWith('>') && (wholeLine.includes('xs:group') || (nearestOpenItem.includes('group')))) {
						vscode.window.activeTextEditor?.insertSnippet(new vscode.SnippetString('>\n\t$0\n</xs:group>'), backpos);
					}
					else if((wholeLine.endsWith('>') && wholeLine.includes('xs:sequence') || (nearestOpenItem.includes('sequence')))) {
						vscode.window.activeTextEditor?.insertSnippet(new vscode.SnippetString('>\n\t$0\n</xs:sequence>'), backpos);
					}
					else if(wholeLine.endsWith('>') && (wholeLine.includes('xs:simpleType')) || (nearestOpenItem.includes('simpleType'))) {
	...
	const closeElementSlashProvider = vscode.languages.registerCompletionItemProvider(
		'dfdl',
		{
			provideCompletionItems(document: vscode.TextDocument, position: vscode.Position) {
				var backpos = position.with(position.line, position.character - 1);
	...
	const endSingleBraceProvider = vscode.languages.registerCompletionItemProvider (
		'dfdl',
		{
			provideCompletionItems(document: vscode.TextDocument, position: vscode.Position) {
	...
	function getDefinedTypes(document: vscode.TextDocument) {
		...
		while(lineNum !== lineCount) {
			...
			if(wholeLine.includes('xs:simpleType Name=') || wholeLine.includes('xs:complexType Name=')) {
